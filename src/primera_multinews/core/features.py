...
def compute_minicheck_utility_scores(unique_sentences, article, minicheck_scorer):
    ...
    try:
        ...
        if "expected a non-empty list of Tensors" not in str(exc):
            raise
        print(
            "[MiniCheck utility] batch scoring returned no tensors; "
            "falling back to per-sentence scoring with 0.0 for failed claims."
        )

    scores = []
    for claim in claims:
        try:
            _, raw_probs, _, _ = minicheck_scorer.score(docs=[article], claims=[claim])
            scores.append(float(raw_probs[0]) if raw_probs else 0.0)
        except ValueError as exc:
            if "expected a non-empty list of Tensors" not in str(exc):
                raise
            scores.append(0.0)
