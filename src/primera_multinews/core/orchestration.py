"""Experiment orchestration for PRIMERA-MultiNews generate-then-optimize runs."""
...
from core.beam_search import batch_beam_search_decode
...
def _format_multinews_source(article: str) -> str:
    documents = [doc.strip() for doc in article.split("|||||") if doc.strip()]
    if not documents:
        return article.strip()
    return f" {DOC_SEPARATOR_TOKEN} ".join(documents)
...
def _build_result_paths(args, method, objective_name, is_any_baseline):
    ...
    if args.output_dir:
        ...
        save_dir = os.path.join(
            ...
            "results",
            generator,
            getattr(args, "dataset", "multi_news"),
            f"{method}_{objective_name}",
        )
    os.makedirs(save_dir, exist_ok=True)

    result_name_parts = [f"beam{args.beam_size}", method]
...
def _run_experiment_impl(args) -> str:
    ...
    if ordering_method not in ORDERING_CHOICES:
        ...
        sys.exit(1)
    if getattr(args, "use_local_rouge", False):
        print("Error: PRIMERA-MultiNews only supports HuggingFace evaluate ROUGE.")
        sys.exit(1)
    rouge_impl = DEFAULT_ROUGE_IMPL
    rouge_impl_desc = "huggingface_evaluate"
    rouge_sentence_split = getattr(args, "rouge_sentence_split", "nltk")
    if rouge_sentence_split != "nltk":
        print("Error: PRIMERA-MultiNews fixes ROUGE sentence splitting to nltk for HF protocol alignment.")
        sys.exit(1)
    ...
    dataset_name = getattr(args, "dataset", "multi_news")
    ...
    effective_max_decode = None
    effective_min_decode = None
    ...
    if resolved_reuse_stage1_from:
        print(
            f"{GENERATOR_DISPLAY_NAME} candidates reused from file, Budget={budget_desc}, "
            f"selector rerun only\n"
        )
    else:
        print(
            f"{GENERATOR_DISPLAY_NAME} beam={beam_size}, Budget={budget_desc}, "
            f"MaxInputTokens={getattr(args, 'max_input_tokens', MAX_INPUT_TOKENS)}, "
            f"MaxDecode={effective_max_decode}, MinDecode={effective_min_decode}"
        )
        print(
            f"LenPenalty={effective_length_penalty}, "
            f"NoRepeatNgram={effective_no_repeat_ngram}, "
            f"EarlyStopping={effective_early_stopping}\n"
        )
    ...
    if reuse_stage1_from:
        ...
        print(f"Loading {GENERATOR_DISPLAY_NAME} model...")
        tokenizer = AutoTokenizer.from_pretrained(model_path)
        model_kwargs = {}
        if device.type == "cuda" and compute_dtype is not None:
            model_kwargs["torch_dtype"] = compute_dtype
        model = LEDForConditionalGeneration.from_pretrained(model_path, **model_kwargs).to(device)
        model.eval()
        generation_config = model.generation_config
        task_specific_params = getattr(model.config, "task_specific_params", {}) or {}
        summarization_defaults = task_specific_params.get("summarization", {}) or {}
        effective_max_decode = generation_config.max_length or summarization_defaults.get("max_length")
        effective_min_decode = generation_config.min_length or summarization_defaults.get("min_length")
        effective_length_penalty = generation_config.length_penalty or summarization_defaults.get("length_penalty")
        effective_no_repeat_ngram = (
            generation_config.no_repeat_ngram_size or summarization_defaults.get("no_repeat_ngram_size")
        )
        effective_early_stopping = generation_config.early_stopping
        if effective_early_stopping is None:
            effective_early_stopping = summarization_defaults.get("early_stopping", True)
    ...
    def _process_sample(idx: int, article: str, candidates, reuse_row: dict | None = None) -> None:
        ...
        if reuse_row is not None and reuse_row.get("prompt_version") is not None:
            sample_log["prompt_version"] = reuse_row.get("prompt_version")
    ...
    try:
        if reuse_stage1_rows is not None:
            ...
            for batch_start in range(first_batch_start, num_samples, generation_batch_size):
                ...
                formatted_articles = [_format_multinews_source(article) for article in batch_articles]
                ...
                with _autocast_context(device, compute_dtype):
                    batch_candidates = batch_beam_search_decode(
                        model,
    ...
    generation_title = f"PRIMERA Beam-{beam_size}"
    generation_detail = (
        f"DocSeparatorToken: {DOC_SEPARATOR_TOKEN}\n"
        f"LenPenalty: {effective_length_penalty}\n"
        f"NoRepeatNgram: {effective_no_repeat_ngram}\n"
        f"EarlyStopping: {effective_early_stopping}\n"
        f"MaxDecode: {effective_max_decode}, MinDecode: {effective_min_decode}\n"
