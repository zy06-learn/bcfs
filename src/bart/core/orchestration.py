"""Experiment orchestration for BART-large-CNN generate-then-optimize runs."""
...
def _resolve_runtime_profile(args, device: torch.device, beam_size: int):
    ...
    if big_gpu:
        auto_generation_batch = 24 if beam_size <= 4 else 12
        auto_utility_batch = 128
        auto_eval_batch = 64
    else:
...
def _run_experiment_impl(args) -> str:
    ...
    model = BartForConditionalGeneration.from_pretrained(model_path, **model_kwargs).to(device)
    model.eval()
    ...
    print(
        ...
        f"MaxDecode={generation_config.max_length}, MinDecode={generation_config.min_length}"
    )
    print(
        f"LenPenalty={generation_config.length_penalty}, "
        f"NoRepeatNgram={generation_config.no_repeat_ngram_size}, "
        f"EarlyStopping={generation_config.early_stopping}\n"
    )
    ...
    try:
        ...
        for batch_start in range(first_batch_start, num_samples, generation_batch_size):
            ...
            for local_offset, (article, candidates) in enumerate(zip(batch_articles, batch_candidates)):
                ...
                if idx < start_index:
                    continue  # resumed: sample already generated and persisted in prior run
                ...
                sample_log["generator_backend"] = generator
                sample_log["generator_model"] = model_path
                sample_log["generator_candidate_count"] = 1 if is_generation_baseline else beam_size
    ...
    generation_title = f"BART Beam-{beam_size}"
    generation_detail = (
        f"LenPenalty: {generation_config.length_penalty}, "
        f"NoRepeatNgram: {generation_config.no_repeat_ngram_size}, "
        f"EarlyStopping: {generation_config.early_stopping}\n"
