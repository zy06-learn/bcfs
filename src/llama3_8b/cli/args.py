...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Meta-Llama-3-8B-Instruct generate-then-optimize summarization experiments")
    parser.add_argument(
        "--method",
        required=True,
        choices=ALL_METHODS,
        help="Optimization method to run for this Meta-Llama-3.1-8B-Instruct experiment directory.",
    )
    parser.add_argument(
        "--objective",
        default=None,
    ...
    parser.add_argument(
        ...
        help=f"Sampling top-k (default: {GENERATION_TOP_K}; 0 = disabled).",
    )
    parser.add_argument(
        "--min-p",
        type=float,
    ...
    parser.add_argument(
        ...
        default="bf16",
        help="Inference dtype for the generation stage (default: bf16 for Meta-Llama-3-8B-Instruct).",
    )
    parser.add_argument(
        "--ordering",
